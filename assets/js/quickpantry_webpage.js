...
window.rejectFile = async function (id) {
  await deleteDoc(doc(db, "pending_uploads", id));
  alert("Rejected");
};
...
function loadPending() {
  ...
  onSnapshot(collection(db, "pending_uploads"), (snap) => {
    ...
    list.innerHTML = snap.docs.map(d => {
      ...
      return `
        <div class="card">
          ...
          <button onclick='approveFile("${d.id}", ${JSON.stringify(data)})'>
            Approve
          </button>

          <button onclick='rejectFile("${d.id}")'>
            Reject
          </button>
        </div>
      `;
